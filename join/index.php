...
session_start();
require('../library.php');
if(isset($_GET['action'])&& $_GET['action']==='rewrite' && isset($_SESSION['form'])){ //書き直した時の値保持
    $form=$_SESSION['form'];
}else{
$form=[ //空にする
    'name'=>'',
...
} else if (strlen($form['password'])<4){ //パスワードの何文字以上のフィルター
...
if($type !=='image/png' && $type !== 'image/jpeg'){
 ...
 if(empty($error)){ //errorに値が廃いているとfalseになる処理
    $_SESSION['form']=$form;
    //画像のアップロード
    if ($image['name'] !== ''){
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <dl>
                ...
                <dd>
                    ...
                    <?php if (isset($error['name']) && $error['name']==='blank'): //issetでエラーがない時にエラーメッセージを表示しない?>
